...
import (
	izappunit "agnione/v2/src/aau/iappunit"
	...
	unitbase "agnione/v2/src/aau/base"
	izappfm "agnione/v2/src/appfm/iappfw"
)
...
type section struct {
	...
	Url            string `json:"url"`
	Frequency_Secs int    `json:"frequency_secs"`
}
...
func (appc *AUDemoHttp) Initialize(pFM_Instance izappfm.IAgniApp, pInstance_ID int,
	...
	if _err != nil {
		...
		//// send monitor message to websocket monitor clients
	...
	if _err != nil {
		_info := appc.base.App_UID + " - Application Unit failed to parse the config file " + pConfig_File + _err.Error()
		appc.base.Write2Log(_info, fmtypes.LOG_ERROR)
...
func (appc *AUDemoHttp) Start() (bool, error) {
	...
	appc.base.Stopper = make(chan bool) //// main chanel to control units's routines

	//// start the routines that does perform tasks
...
func (appc *AUDemoHttp) Stop() (bool, error) {
	...
	appc.base.AppFramework.Write2Log(appc.base.App_UID+" - Stopping HTTP demo......", fmtypes.LOG_INFO)
	if appc.base.Stopper != nil {
		close(appc.base.Stopper)
	}

	appc.base.AppFramework.Write2Log(appc.base.App_UID+" - Stopping HTTP demo........... DONE", fmtypes.LOG_INFO)
	return true, nil
}
